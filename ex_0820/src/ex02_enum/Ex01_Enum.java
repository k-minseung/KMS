...
public class Ex01_Enum {
	public static void main(String[] args) {
		...
		//ordinal() -> 열거 타입을 정의할 때 주어진 순번을 말함
		for(Item item : items) {
		...
		Item i3 = Item.STOP;
		//열거형의 상수간 비교는 == 을 사용한다. (상수끼리의 주소를 비교)
		System.out.println(i1==i2);
		
		//초과,미만,이상,이화와 같은 비교연산자는 사용할 수 없다.
		//compareTo();
		//매개값을 주어진 객체를 비교해서 순번 차이를 반환
		//기준객체가 비교객체보다 순번이 빠르면 음수 반환
		//기준객체가 비교객체보다 순번이 느리면 양수 반환
		...
		case STOP -> System.out.println("게임정지");
