...
public class Ex07_arrays {
	public static void main(String[] args) {
		...
		//더 많은 데이터를 저장하거나 기존의 배열과 같은 배열을 새로 만드려면 배열을 복사해랴한다.
		//베열을 복사하는 방법에는 얕은 복사와 깊은 복사 두가지가 있다.
		
		//얕은복사(Shallow Copy) : 복사된 배열이나 원본 배열이 변경 될 때 서로간의 값이 함께 변경된다.
		
		//깊은복사(Deep Copy) : 복사된 배열이나 원본 배열이 서로 변경될 떄 서로 간의 값은 바뀌지 않는다.
		...
		//깊은복사
		//배열의 깊은복사는 반복문을 이용해 새로운 배열에 값을 직접 넣어주거나 Arrays클래스 또는  System클래스가 가진 기능을 이용한다.
